...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updatedinningQuery = "UPDATE dinning SET name=?, code=?, type=?, price=?, offer_price=?, material=?, size=?, information=? WHERE id=?";
            $stmtdinning = mysqli_prepare($conn, $updatedinningQuery);
            mysqli_stmt_bind_param($stmtdinning, "ssssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $offerprice, $materials, $size, $information, $id);
