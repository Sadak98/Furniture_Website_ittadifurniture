...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updateofficeQuery = "UPDATE office SET name=?, code=?, type=?, price=?, offer_price=?, material=?, size=?, information=? WHERE id=?";
            $stmtoffice = mysqli_prepare($conn, $updateofficeQuery);
            mysqli_stmt_bind_param($stmtoffice, "ssssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $offerprice, $materials, $size, $information, $id);
