...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            $updateBedroomQuery = "UPDATE bedroom SET name=?, code=?, type=?, price=?, offer_price=?, material=?, size=?, information=? WHERE id=?";
            $stmtBedroom = mysqli_prepare($conn, $updateBedroomQuery);
            mysqli_stmt_bind_param($stmtBedroom, "ssssssssi", $furnitureName, $furnitureCode, $furnitureType, $price, $offerprice, $materials, $size, $information, $id);
